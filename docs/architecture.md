...
- **No unique `userId`.** Functional Requirement 3 calls for a specific `userId`'s score and rank, but `LeaderboardEntry` currently only has `playerName`, a free-text string with no identity behind it — two submissions from the same real player aren't linked, and two different players who pick the same name collide. This also undermines idempotency and the write-queue dedupe design above, both of which assume a stable per-player key (`gameId + userId`); right now they can only dedupe on whatever the client happens to put in `playerName`. A real `userId` (issued by an auth system, not client-chosen text) is a prerequisite for those features to work correctly.
